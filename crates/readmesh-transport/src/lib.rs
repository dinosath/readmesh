...
use bytes::Bytes;
use readmesh_metalink::{HashType, Metalink, MetalinkError};
...
mod tests {
    use super::*;
    use async_trait::async_trait;
    use readmesh_metalink::{HashType, Metalink};
    ...
    impl MockBackend {
        fn new(scheme: &'static str, priority: BackendPriority) -> Self {
            Self {
                scheme,
                priority,
        ...
        fn with_data(mut self, url: &str, data: &[u8]) -> Self {
            self.data_map
                .insert(url.to_string(), Bytes::from(data.to_vec()));
            self
        }
    }
    ...
    async fn resolver_tries_backends_in_priority_order() {
        ...
        let low_prio =
            MockBackend::new("low", BackendPriority::Fallback).with_data("low://test", b"low-data");
