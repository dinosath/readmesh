    ...
    match client.request(RpcRequest::GetLibrary).await {
        RpcResponse::Library { novels } => {
            if novels.is_empty() {
                ...
                println!("Use 'lnreader search <plugin> <query>' to find novels.");
                return Ok(());
            }
            println!("Library ({novels_len} novels):", novels_len = novels.len());
            for novel in &novels {
                let status = match novel.status {
                    ln_core::novel::NovelStatus::Ongoing => "[ongoing]",
                    ln_core::novel::NovelStatus::Completed => "[completed]",
                    ln_core::novel::NovelStatus::Hiatus => "[hiatus]",
                    ln_core::novel::NovelStatus::Dropped => "[dropped]",
                    ln_core::novel::NovelStatus::Unknown => "[unknown]",
                };
                let authors: Vec<String> =
                    novel.authors.iter().map(|a| a.name.clone()).collect();
                println!(
                    "  {} - {}  {status}",
                ...
                if let Some(ref summary) = novel.summary {
                    ...
                    println!("    {short}{}", if summary.len() > 120 { "..." } else { "" });
                }
            }
        }
...
pub async fn cmd_search(
    ...
    match response {
        RpcResponse::SearchResults { results } => {
            ...
            println!("Search results for '{query}' ({results_len}):", results_len = results.len());
            for novel in &results {
                println!("  {} - {}", novel.id, novel.title);
                if let Some(ref summary) = novel.summary {
                    let short: String = summary.chars().take(100).collect();
                    println!("    {short}{}", if summary.len() > 100 { "..." } else { "" });
                }
            }
            println!();
...
pub async fn cmd_daemon(_data_dir: &str) -> anyhow::Result<()> {
    println!("Daemon mode: run 'lnreaderd' directly instead.");
    println!("  cargo run --bin lnreaderd -- --data-dir ./lnreader-data");
    Ok(())
}
...
pub async fn cmd_federation(client: &DaemonClient) -> anyhow::Result<()> {
    ...
    match response {
        RpcResponse::FederationStatus { status } => {
            ...
            println!();
            println!("Stat:");
            println!("  Mirrored: {} bytes / {} chapters",
...
pub async fn cmd_unfollow_peer(
    client: &DaemonClient,
    node_id_hex: &str,
) -> anyhow::Result<()> {
    let node_id_bytes = hex::decode(node_id_hex)?;
