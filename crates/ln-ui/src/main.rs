...
use commands::{
    cmd_chapters, cmd_daemon, cmd_federation, cmd_follow_peer, cmd_library, cmd_plugins,
    cmd_read, cmd_search, cmd_unfollow_peer,
};

#[derive(Parser)]
#[command(name = "lnreader", about = "lnreader-rs CLI client")]
struct Cli {
    #[command(subcommand)]
    command: Command,
    ...
    tracing_subscriber::fmt()
        .with_env_filter(
            tracing_subscriber::EnvFilter::from_default_env()
                .add_directive("lnreader=info".parse()?),
        )
        .init();
