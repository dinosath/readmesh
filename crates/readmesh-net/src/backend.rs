                ...
                loop {
                        ...
                        && let Some(announcement) = guard.get(current_len - 1).cloned()
                    {
                        ...
                        for sender in &senders {
                            let _ = sender.send(P2pEvent::Announce(announcement.clone())).await;
                        }
...
mod tests {
    ...
    async fn cluster_announce_to_all() {
        ...
        let announcement = PeerAnnouncement {
            ...
            novel_id,
            chapter_id: readmesh_core::id::ChapterId(blake3::Hash::from([0u8; 32])),
        ...
        let event2 =
            tokio::time::timeout(tokio::time::Duration::from_millis(100), node2.next_event())
                .await
                .unwrap()
                .unwrap();
