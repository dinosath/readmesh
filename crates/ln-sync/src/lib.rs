...
//!
//! Uses ln-net's gossip layer to synchronize a user's own devices.
...
impl SyncEngine {
    ...
    pub async fn broadcast_library_add(
        &self,
        novel: &Novel,
    ) -> Result<(), SyncError> {
    ...
    pub async fn broadcast_library_remove(
        &self,
        novel_id: &NovelId,
    ) -> Result<(), SyncError> {
        let now = Utc::now();
        ...
        let _ = self.outgoing_tx.try_send(SyncMessage::LibraryRemove(*novel_id));
        Ok(())
    }
        ...
        match msg {
            SyncMessage::ProgressUpdate(progress) => {
                ...
                if is_newer {
                    state
                        .progress
                        .insert(key, (timestamp, progress.clone()));
                }
                is_newer
            }
...
mod tests {
    ...
    fn make_chapter(novel_id: NovelId, index: u32) -> ChapterId {
        ChapterId::compute(
            &format!("https://test.com/ch{index}"),
            &novel_id,
            index,
        )
    }
