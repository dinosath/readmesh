...
//! via `readmesh-store` but provides the authoritative in-memory state.
...
impl Library {
    ...
    pub fn remove_novel(&mut self, novel_id: &NovelId) -> Option<Novel> {
        self.chapters.retain(|_, ch| ch.novel_id != *novel_id);
    ...
    pub fn get_progress(&self, novel_id: &NovelId, device_id: &NodeId) -> Option<&ReadingProgress> {
...
mod tests {
    ...
    fn library_novels_by_tag() {
        let mut lib = Library::new();
        lib.add_novel(Novel::new("Fantasy Novel", "https://test.com/fantasy").with_tag("Fantasy"));
        lib.add_novel(Novel::new("Sci-Fi Novel", "https://test.com/scifi").with_tag("Sci-Fi"));
