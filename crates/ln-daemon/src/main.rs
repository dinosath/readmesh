//! Headless node daemon (`lnreaderd`).

use std::path::PathBuf;

use clap::Parser;
use ln_daemon::service::DaemonService;
use tokio::io::{AsyncBufReadExt, AsyncWriteExt, BufReader};
use tokio::net::TcpListener;

#[derive(Parser, Debug)]
#[command(name = "lnreaderd", about = "lnreader-rs headless daemon")]
struct Args {
    /// Data directory
    #[arg(short, long, default_value = "./lnreader-data")]
    data_dir: PathBuf,
    ...
    tracing_subscriber::fmt()
        .with_env_filter(
            tracing_subscriber::EnvFilter::from_default_env()
                .add_directive("lnreaderd=info".parse()?),
        )
        .init();

    let args = Args::parse();
    tracing::info!("Starting lnreaderd with data dir: {}", args.data_dir.display());

    // Ensure data directory exists
    std::fs::create_dir_all(&args.data_dir)?;

    let service =
        DaemonService::new(&args.data_dir).await?;
                ...
                result = lines.next_line() => {
                    match result {
                        Ok(Some(line)) => {
                            ...
                            let request: ln_rpc::RpcRequest = match serde_json::from_str(&line) {
                                Ok(req) => req,
                                Err(e) => {
                                    let err = ln_rpc::RpcResponse::Error {
