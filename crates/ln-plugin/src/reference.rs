...
use ln_core::id::PluginId;
use ln_core::novel::{Novel, NovelStatus};
use ln_core::source::{PluginCapabilities, PluginManifest};
...
impl ReferencePlugin {
    fn catalog() -> Vec<Novel> {
        vec![
            Novel::new(
                "Test Novel One",
                "https://example.com/novels/test-one",
            )
            ...
            Novel::new(
                "Test Novel Two",
                "https://example.com/novels/test-two",
            )
            ...
            Novel::new(
                "Test Novel Three",
                "https://example.com/novels/test-three",
            )
    ...
    fn chapters_for_novel(novel_id: ln_core::id::NovelId) -> Vec<Chapter> {
        (0..5)
            .map(|i| {
...
mod tests {
    ...
    async fn fetch_novel_not_found() {
        let plugin = ReferencePlugin::new();
        let result = plugin.fetch_novel("https://example.com/novels/nonexistent").await;
