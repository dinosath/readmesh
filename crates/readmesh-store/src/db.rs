...
impl Database {
    ...
    pub fn get_novel(&self, id: &NovelId) -> Result<Option<Novel>, StoreError> {
        ...
        let result = stmt
            .query_row(params![hash_as_blob(&id.0)], |row| {
                ...
                let authors = serde_json::from_str(&authors_json).map_err(|e| {
                ...
                let source_refs = serde_json::from_str(&source_refs_json).map_err(|e| {
                    rusqlite::Error::FromSqlConversionFailure(
                        0,
                        rusqlite::types::Type::Text,
                        Box::new(e),
                ...
                let added_at = text_to_datetime(&added_at_str).unwrap_or_else(Utc::now);
                let updated_at = text_to_datetime(&updated_at_str).unwrap_or_else(Utc::now);
    ...
    pub fn list_novels(&self) -> Result<Vec<Novel>, StoreError> {
        let ids: Vec<Vec<u8>> = {
            let conn = self.lock();
            let mut stmt = conn.prepare("SELECT id FROM novels ORDER BY updated_at DESC")?;
    ...
    pub fn insert_chapter(&self, chapter: &Chapter) -> Result<(), StoreError> {
        let metalink_blob: Option<&[u8]> = chapter.metalink_hash.as_ref().map(hash_as_blob);
        let published = chapter.published_at.as_ref().map(datetime_to_text);
    ...
    pub fn list_chapters_for_novel(&self, novel_id: &NovelId) -> Result<Vec<Chapter>, StoreError> {
        let ids: Vec<Vec<u8>> = {
            let conn = self.lock();
            let mut stmt =
                conn.prepare("SELECT id FROM chapters WHERE novel_id = ?1 ORDER BY idx ASC")?;
    ...
    pub fn get_progress(
        ...
        stmt.query_row(
            ...
            |row| {
                ...
                let device_id = device_id_blob
                    .try_into()
                    .map_err(|_| rusqlite::Error::InvalidColumnName("device_id".into()))?;
                let updated_at = text_to_datetime(&updated_at_str).unwrap_or_else(Utc::now);
    ...
    pub fn list_progress_for_device(
        ...
        let rows: Vec<(Vec<u8>, Vec<u8>)> = {
            let conn = self.lock();
            let mut stmt = conn
                .prepare("SELECT novel_id, device_id FROM reading_progress WHERE device_id = ?1")?;
...
mod tests {
    ...
    fn plugin_crud() {
        ...
        let manifest = PluginManifest {
            id: readmesh_core::id::PluginId("test-plugin".into()),
